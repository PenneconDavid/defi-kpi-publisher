...
var PUBLISH_REPORT_PARAMS = parseAbiParameters("bytes32 protocolId, uint256 tvlUsd, int256 change1dBps, int256 change7dBps, bytes32 runId");
var APPLY_POLICY_REPORT_PARAMS = parseAbiParameters("bytes32 protocolId, uint8 nextMode, bytes32 reasonCode");
function fetchCurrentTvl(sendRequester, config) {
  const response = sendRequester.sendRequest({ method: "GET", url: config.defiLlamaUrl }).result();
...
function evaluatePolicy(kpi, config) {
  const breach = kpi.tvlUsd < config.minTvlUsd || kpi.change1dBps < config.minChange1dBps || kpi.change7dBps < config.minChange7dBps;
  return {
    breach,
    nextMode: breach ? 1 : 0,
...
function executeKpiCycle(runtime2, triggerName) {
  const config = runtime2.config;
  const evmConfig = config.evms[0];
  const protocolId = config.protocolIdHex;
  const runId = generateRunId(triggerName);
  runtime2.log(`[KPI Cycle] Trigger: ${triggerName}`);
  runtime2.log(`[KPI Cycle] Protocol: ${protocolId}`);
  runtime2.log("[Step 1] Fetching current TVL from DefiLlama...");
  const httpClient = new cre.capabilities.HTTPClient;
  const tvlData = httpClient.sendRequest(runtime2, fetchCurrentTvl, ConsensusAggregationByFields({
    currentTvl: median
  }))(config).result();
  const currentTvl = Math.max(0, Math.round(tvlData.currentTvl));
  runtime2.log(`[Step 1] Current TVL: ${currentTvl}`);
  runtime2.log("[Step 2] Reading prior snapshot from KpiOracle...");
  ...
  const priorTvl = Number(priorSnapshot[0] || 0n);
  const priorTimestamp = Number(priorSnapshot[3] || 0n);
  runtime2.log(`[Step 2] Prior TVL: ${priorTvl}, timestamp: ${priorTimestamp || "none"}`);
  let change1dBps = 0;
  let change7dBps = 0;
  if (priorTvl > 0) {
  ...
  const kpi = { tvlUsd: currentTvl, change1dBps, change7dBps };
  runtime2.log(`[Step 2] Computed KPI: tvl=${kpi.tvlUsd}, 1d=${kpi.change1dBps}bps, 7d=${kpi.change7dBps}bps`);
  runtime2.log("[Step 3] Publishing new KPI snapshot...");
  ...
  const publishReport = runtime2.report({
    encodedPayload: hexToBase64(publishData),
    encoderName: "evm",
    signingAlgo: "ecdsa",
    hashingAlgo: "keccak256"
  }).result();
  const publishResult = evmClient.writeReport(runtime2, {
    receiver: evmConfig.kpiOracleAddress,
    report: publishReport,
    gasConfig: { gasLimit: evmConfig.gasLimit }
  }).result();
  if (publishResult.txStatus !== TxStatus.SUCCESS) {
    throw new Error(`Publish tx failed: ${publishResult.errorMessage || publishResult.txStatus}`);
  }
  const publishTxHash = bytesToHex(publishResult.txHash || new Uint8Array(32));
  runtime2.log(`[Step 3] Published: ${publishTxHash}`);
  runtime2.log("[Step 4] Evaluating policy thresholds...");
  const outcome = evaluatePolicy(kpi, config);
  runtime2.log(`[Step 4] Breach: ${outcome.breach}, Mode: ${outcome.nextMode === 1 ? "DEFENSIVE" : "NORMAL"}`);
  if (outcome.breach) {
    runtime2.log("[Step 5] Threshold breached, applying policy...");
    const reasonCodeHex = `0x${Buffer.from(outcome.reasonCode).toString("hex").padEnd(64, "0")}`;
    const applyData = encodeAbiParameters(APPLY_POLICY_REPORT_PARAMS, [
      protocolId,
      outcome.nextMode,
      reasonCodeHex
    ]);
    const applyReport = runtime2.report({
      encodedPayload: hexToBase64(applyData),
      encoderName: "evm",
      signingAlgo: "ecdsa",
      hashingAlgo: "keccak256"
    }).result();
    const applyResult = evmClient.writeReport(runtime2, {
      receiver: evmConfig.strategyControllerAddress,
      report: applyReport,
      gasConfig: { gasLimit: evmConfig.gasLimit }
    }).result();
    if (applyResult.txStatus !== TxStatus.SUCCESS) {
      throw new Error(`ApplyPolicy tx failed: ${applyResult.errorMessage || applyResult.txStatus}`);
    }
    const applyTxHash = bytesToHex(applyResult.txHash || new Uint8Array(32));
    runtime2.log(`[Step 5] Policy applied: ${applyTxHash}`);
    return `breach:${applyTxHash}`;
  }
  runtime2.log("[Step 5] No breach, skipping policy application.");
...
function onCronTrigger(runtime2, payload) {
  ...
  runtime2.log("CRE Workflow: Cron Trigger - Proactive KPI Update");
  return executeKpiCycle(runtime2, "cron");
}
...
function onLogTrigger(runtime2, log) {
  runtime2.log("CRE Workflow: Log Trigger - Reactive KPI Update");
  const topics = log.topics.map((t) => bytesToHex(t));
  const data = bytesToHex(log.data);
  const decoded = decodeEventLog({ abi: EVENT_ABI, data, topics });
  const protocolId = decoded.args.protocolId;
  const requester = decoded.args.requester;
  const reason = decoded.args.reason;
  runtime2.log(`[Log Trigger] Protocol: ${protocolId}`);
  runtime2.log(`[Log Trigger] Requester: ${requester}`);
  runtime2.log(`[Log Trigger] Reason: ${reason}`);
  return executeKpiCycle(runtime2, "reactive");
}
var configSchema = exports_external.object({
  schedule: exports_external.string(),
  defiLlamaUrl: exports_external.string(),
  protocolIdHex: exports_external.string(),
  minTvlUsd: exports_external.number(),
  minChange1dBps: exports_external.number(),
  minChange7dBps: exports_external.number(),
  evms: exports_external.array(exports_external.object({
    kpiOracleAddress: exports_external.string(),
    strategyControllerAddress: exports_external.string(),
    chainSelectorName: exports_external.string(),
    gasLimit: exports_external.string()
  }))
});
var UPDATE_REQUESTED_SIGNATURE = "UpdateRequested(bytes32,address,string)";
